# 3-07-02 各ツールの実行例（sample_code.py / type_error_sample.py）のセットアップ / Windows PowerShell
# 何度実行しても同じ状態になる。
...
Write-Host "[$Section] インストール確認（原稿の各ツールのバージョン確認に相当）"
& $venvPython -m black --version
& $venvPython -m flake8 --version
& $venvPython -m mypy --version
& $venvPython -m isort --version

Write-Host "[$Section] セットアップが完了しました"
Write-Host "[$Section] sample_code.py と type_error_sample.py は意図的に問題を含むコードです（各ツールが指摘を出すのが正しい状態です）"
Write-Host "[$Section] 任意検証: .\.venv\Scripts\python.exe -m black --check sample_code.py"
Write-Host "[$Section] 任意検証: .\.venv\Scripts\python.exe -m flake8 sample_code.py / -m mypy type_error_sample.py"
